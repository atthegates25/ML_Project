...
#residuals
train_resid = y_train - ols_train_predictions
plt.scatter(y_train, train_resid)

#feature selection
from sklearn.preprocessing import StandardScaler

var_thresholds = np.linspace(0.1, 1, 20)
ols_fs_varth_train_rmse = []
ols_fs_varth_test_rmse = []
ols_fs_varth_nvars = []
for t in var_thresholds:
    X_train_fs = pd.DataFrame(fs.VarianceThreshold(threshold = t).fit_transform(features))
    cols = fs.VarianceThreshold(threshold = t).fit(features).get_support()
    col_names = X_train.columns[cols]
    ols_fs_varth_nvars.append(X_train_fs.shape[1])
    ols_fs = linear_model.LinearRegression()
    ols_fs.fit(X_train_fs, y_train)
    ols_fs_varth_train_rmse.append(mean_squared_error(y_train, ols_fs.predict(X_train_fs)) ** (1/2))
    X_test_fs = X_test.loc[:, col_names]
    ols_fs_varth_test_rmse.append(mean_squared_error(y_test, ols_fs.predict(X_test_fs)) ** (1/2))
ols_fs_varth_train_rmse
ols_fs_varth_test_rmse
ols_fs_varth_nvars

varth_rdata_rmse = pd.DataFrame({"train_mse": ols_fs_varth_train_rmse,
                                 "test_mse": ols_fs_varth_test_rmse}, index = var_thresholds)
varth_rdata_nvars = pd.DataFrame({"nvars": ols_fs_varth_nvars}, index = var_thresholds)

#ols rmse by threshold
title1 = "Feature selection with OLS - variance thresholds"
axes = varth_rdata_rmse.plot(logx = False, title = title1, legend = False)
axes.set_xlabel("variance threshold")
axes.set_ylabel("rmse")
plt.plot()

#no. of variables by threshold
title2 = "Feature selection with OLS - variance thresholds"
axes = varth_rdata_nvars.plot(logx = False, title = title2, legend = False)
axes.set_xlabel("variance threshold")
axes.set_ylabel("no. of variables")
plt.plot()
